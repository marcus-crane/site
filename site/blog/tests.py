...
class PostIndexViewTests(TestCase):
  ...
  def test_draft_post(self):
    ...
    """
    draft_post = create_post(title='Draft Post', text='This is a draft post', days=None, status='D')
    blog = self.client.get(reverse('blog:list'))
    self.assertEqual(blog.status_code, 200)
    self.assertQuerysetEqual(blog.context['posts'], [])
  ...
  def test_unlisted_post(self):
    ...
    """
    unlisted_post = create_post(title='Draft Post', text='This is a draft post', days=None, status='U')
    blog = self.client.get(reverse('blog:list'))
    self.assertEqual(blog.status_code, 200)
    self.assertQuerysetEqual(blog.context['posts'], [])
  ...
  def test_published_post(self):
    ...
    """
    public_post = create_post(title='Public Post', text='This is a public post', days=-2, status='P')
    blog = self.client.get(reverse('blog:list'))
    self.assertEqual(blog.status_code, 200)
  ...
  def test_publish_draft_post(self):
    ...
    """
    draft_post = create_post(title='Draft Post', text='This will be published', days=None, status='D')
    blog = self.client.get(reverse('blog:list'))
    self.assertQuerysetEqual(blog.context['posts'], [])
  ...
  def test_published_post_with_future_date(self):
    ...
    """
    public_post = create_post(title='Future Post', text='Hello from the future!', days=2, status='P')
    blog = self.client.get(reverse('blog:list'))
    self.assertEqual(blog.status_code, 200)
    self.assertContains(blog, 'No posts have been written yet!')
    self.assertQuerysetEqual(blog.context['posts'], [])